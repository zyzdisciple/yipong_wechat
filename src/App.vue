<template>
  <div id="app" v-show="hasRedirect">
    ...
    <attention-popup v-if="showAttention"></attention-popup>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      refreshUrl(newValue, oldValue) {
        ...
        this.setPopExclusion(newValue.name);
      },
      ...
      setPopExclusion(currentName) {
        let exclusionRoutes = ['视频详情'];
        if (exclusionRoutes.includes(currentName)) {
          this.popExclusion = true;
        } else {
          this.popExclusion = false;
        }
      },
    },
    mounted() {
      ...
      this.setPopExclusion(this.$route.name);
      if (window.localStorage.getItem("hasRegisted") === "false" && !this.registed) {
        this.showModalLogin();
      }
    },
    computed: {
      ...
      registed() {
          //初始exclusionToPop为false, 当需要忽略popExclusion 时, 置为true
          if (this.popExclusion === false || this.$store.state.exclusionToPop) {
              if (window.localStorage.getItem("hasRegisted") === "true" || this.$store.state.isRegisted) {
                  return true;
              } else if (window.localStorage.getItem("hasRegisted") === "false") {
                  return false;
              }
          } else {
            return true;
      ...
      showAttention() {
          //当且仅当 用户已关注, 未注册时展示关注窗口,且刷新之后不要求显示二维码
          if (this.$store.state.isRegisted === true && !this.popExclusion) {
