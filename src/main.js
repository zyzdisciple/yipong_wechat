...
if (currentUrl.indexOf("?url=") != -1) {
  ...
  new Vue({
    ...
    wechatAuth: new VWechatAuth({
        ...
        authorize () {
          let data = {};
          //这段代码用来二次获取用户信息, 将reloadUserInfo 置为true , 同时调用window.wechatAuth.authorize()即可. 且仅在重新获取数据时使用
            ...
            api.wechatcodelogin({
              ...
              if (res.Data != null && res.Data.Token) {
                ...
                api.getRegisted({}).then(res => {
                  if (res.Data) {
                    store.state.isRegisted = res.Data.IsRegisted;
                    window.localStorage.setItem('hasRegisted', store.state.isRegisted);
                  }
                })
