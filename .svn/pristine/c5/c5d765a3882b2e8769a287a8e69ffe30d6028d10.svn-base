      ...
      <div class="plbar-1" style="width: 70%">
        <div class="pl-input">
          <form @submit.prevent="submit">
            <input v-model="comments" class="pl-input1" style="width:55%">
            <img src="../../images/pl-bj.png" class="pl-bj" style="left:6%">
  ...
  export default {
    ...
    mounted() {
      ...
      this.api.getLiveStreamById(paramsLive).then(res => {
        ...
        let startTime = res.Data.StartDateTime;
        this.$store.state.shareData.description = `${liveName}的直播将于${startTime}开始`;
      ...
      loadComments(size) {
        ...
        this.api.getMessages(params).then(res => {
          ...
          tempList.forEach(item => {
            if (!item.commenterHeaderImgUrl) {
              item.commenterHeaderImgUrl = this.$store.state.yipong.UserHeaderImageUrl;
            }
